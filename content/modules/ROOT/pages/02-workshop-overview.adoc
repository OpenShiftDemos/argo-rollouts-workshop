...
* _user{user}-dev_. This represents the development environment where we will deploy the application as a vanilla Kubernetes Deployment so
we can compare and contrast it to Rollouts.
* _user{user}-prod_. This is our production environment, here we will deploy a Rollout for the application. During the course of the workshop
you will deploy rollouts using different strategies and analysis.
* _user{user}-tools_. This is where the CI/CD tooling is located, it contains a pipeline to promote image changes across the development and production
