...
* _{user}-dev_. This represents the development environment where we will deploy the application as a vanilla Kubernetes Deployment so
we can compare and contrast it to Rollouts.
* _{user}-prod_. This is our production environment, here we will deploy a Rollout for the application. During the course of the workshop
you will deploy rollouts using different strategies and analysis.
* _{user}-tools_. This is where the CI/CD tooling is located, it contains a pipeline to promote image changes across the development and production
...
----
oc get rolloutmanager argo-rollout -o yaml -n {user}-prod | oc neat
----
...
metadata:
  annotations:
    argocd.argoproj.io/tracking-id: {user}:argoproj.io/RolloutManager:{user}-prod/argo-rollout
  name: argo-rollout
  namespace: {user}-prod

As shown here this is a simple declaration which in turn results in the Operator creating
a deployment with the Argo Rollout controller monitoring the `{user}-prod` for
`Rollout` resources.
...
[source,bash,subs="attributes+,+macros"]
----
git clone -b Summit2024 --single-branch https://github.com/OpenShiftDemos/argo-rollouts-workshop ~/argo-rollouts-workshop
----

[NOTE]
We are cloning the Summit2024 branch to support some specific requirements of the Summit environment.
...
.link:https://github.com/OpenShiftDemos/argo-rollouts-workshop/blob/main/content/modules/ROOT/examples/deploy/base/deploy.yaml[./deploy/base/deploy.yaml,window='_blank']
[source,yaml,subs="+macros,attributes+"]
----
include::ROOT:example$deploy/base/deploy.yaml[]
----

.link:https://github.com/OpenShiftDemos/argo-rollouts-workshop/blob/main/content/modules/ROOT/examples/deploy/base/service.yaml[./deploy/base/service.yaml,window='_blank']
[source,yaml,subs="+macros,attributes+"]
----
include::ROOT:example$deploy/base/service.yaml[]
----

.link:https://github.com/OpenShiftDemos/argo-rollouts-workshop/blob/main/content/modules/ROOT/examples/deploy/base/route.yaml[./deploy/base/route.yaml,window='_blank']
[source,yaml,subs="+macros,attributes+"]
----
...
.link:https://github.com/OpenShiftDemos/argo-rollouts-workshop/blob/main/content/modules/ROOT/examples/deploy/base/kustomization.yaml[./deploy/base/kustomization.yaml,window='_blank']
[source,yaml,subs="+macros,attributes+"]
----
