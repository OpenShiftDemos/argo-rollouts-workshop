...
To access this pipeline, in the link:{console_url}/dev-pipelines/ns/{user}-tools[OpenShift Console^], ensure you are in the `{user}-tools` namespace
and  `Pipelines` view as per this screenshot.
...
Pipelines, an implementation of the link:https://tekton.dev[Tekton,window='_blank'] project, however
any Continuous Integration(CI) tool can be used with Argo Rollouts including Argo Workflows,
