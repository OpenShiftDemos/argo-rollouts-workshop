...
In the `args` we are taking four arguments, `route-name`, `route-url`, `namespace` and `api-token`, which will be passed from the
Rollout.
...
Finally a fourth parameter, `api-token`, is provided by a secret. This secret provides the token needed
...
is shown with a blue bubble indicating the analysis is in progress.
...
Clicking on the analysis button will show a popup with details of the analysis, clicking
on _Summary_ will show the in-progress analysis that is running:

image::argo-rollouts-analysis-in-progress-details.png[]

Once the analysis is complete the Analysis bubble will go green to show that it successfully
completed:

image::argo-rollouts-analysis-completed.png[]

Hovering the mouse over the analysis button will display
overall status information:

image::argo-rollouts-analysis-in-progress-hover.png[]

If you click on the Analysis button again, the popup will show green bubbles for all
of the analysis steps indicating it was successful. If you click on the Summary note that
four measurements were taken during the course of this Analysis.

image::argo-rollouts-analysis-in-progress-details-completed.png[]
...
Clicking on the Analysis button will show the results of the test, note the red dot under
the success-rate.

image::argo-rollouts-analysis-failed-details.png[]

If you hover the mouse over the dot you can see the results of the
prometheus metric used indicating the number of errors that have occurred.

image::argo-rollouts-analysis-failed-details-hover.png[]
