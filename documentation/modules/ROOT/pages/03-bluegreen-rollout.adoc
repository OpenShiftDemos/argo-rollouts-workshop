...
for installing the extension are available here however note that this has already been done for you in this workshop.
...
To view the rollout we just deployed, run the following command:
...
With this command we can view the status of the rollout, `Healthy` here, as well as
the ReplicaSets and Pods that are associated with the rollout.
...
This should return a simple `Healthy` in the output. Other commands are available to perform
actions on the rollout such as a promoting a rollout, aborting a rollout, etc. A complete
list of commands can be viewed with the `--help` switch and we will look at some of them
...
1. For the CLI, execute the following command to perform the promotion:
...
2. For the UI, click the `Promote` button in the Dashboard, it will change to an orange `Sure?` button
...
Now that a manual promotion of rollout has been reviewed, let's enable the auto promotion feature. To do so, execute
