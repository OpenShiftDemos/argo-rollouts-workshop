...
Similar to Kubernetes deployments, each revision, i.e. version, of the Rollout is backed by a ReplicaSet. This ReplicaSet will have a
`rollouts-pod-template-hash` label in it's PodTemplate with a value unique to the revision. Argo Rollouts will automatically ensure that
...
The Argo Rollout dashboard has been deployed in the `user%USERNUM%-tools` namespace on the cluster for this workshop. To access the dashboard,
run the following command to get the URL for the dashboard for your specific user:

[.console-input]
[source,bash,subs="attributes+,+macros"]
----
oc get route -n user%USERNUM%-tools dashboard -o jsonpath='{"http://"}{.spec.host}{"\n"}'
----
...
basic information about the tile as shown here:
