...
is enabled so the behavior of the application can be observed in more-depth in subsequent sections of this module.
...
will add a unique identifier to pods and then update the selector in the service to bind the right pods to the right service. As
...
Next retrieve the Routes for this application. It's in the `user%USERNUM%-dev1` Project.
...
Notice that the selector now has an additional key, `rollouts-pod-template-hash`, with a unique value. This is how the rollout manages traffic
...
will block when the rollout is not healthy which can be useful when used with
