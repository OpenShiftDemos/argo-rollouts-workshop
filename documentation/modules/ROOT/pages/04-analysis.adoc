...
In the arguments we are taking two key arguments, `query` and `route-url`, which will be passed from the Rollout. Notice that
the `query` argument is being used in the URL of the web provider, this is the Prometheus query that we want to execute
against the OpenShift monitoring stack.
...
The `route-url` parameter is being used in the second provider, the job, where we will be creating load. This parameter is used
to specify the OpenShift route URL where we want to drive the load, i.e the URL that siege will be hitting when it generates load.



A third parameter, `api-token`, is provided by a secret. This secret provides a token needed to access the OpenShift monitoring
