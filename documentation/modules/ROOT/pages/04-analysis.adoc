...
The `count` and `interval` fields in the `success-rate` metric powored by the web provider indicate that the metric will be checked four times
...
With the upodated blue-green rollout deployed, let's run through a promotion to
a new image and observe the behavior. In the OpenShift console, go to the Pipelines
...
Clicking on the analysis button will expand the view to show the in-pregress analysis
that is running:
