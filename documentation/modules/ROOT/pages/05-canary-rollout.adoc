...
[Important]
Argo Rollouts does not currently have support for traffic shaping for OpenShift Routes. As a result
we will be relying on the best effort approach of scaling pods. The `all` service will enable
us to see this by including all pods.
